...
function getOrder($db, $orderId) {
    ...
    $sql = 'Select o.customerName, o.orderFoodName, o.orderDate, o.finishDate, o.total FROM orders o  ';
    $sql .= 'Where o.id = :id';
    $stmt = $db->prepare ($sql);
    $id = (int) $orderId;
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
...
function createOrder($db, $form_data) { 
    //stop at sisni
    $sql = 'Insert into orders ( customerName, orderFoodName, orderDate, finishDate, total)'; 
    $sql .= 'values (:customerName, :orderFoodName, :orderDate, :finishDate, :total)';  
    $stmt = $db->prepare ($sql); 
    $stmt->bindParam(':customerName', $form_data['customerName']);  
    $stmt->bindParam(':orderFoodName', ($form_data['orderFoodName']));
...
function updateOrder($db,$form_dat,$orderId) { 
    ...
    $sql = 'UPDATE orders SET customerName = :customerName , orderFoodName = :orderCakeName , orderDate = :orderDate , finishDate = :finishDate , total = :total'; 
    $sql .=' WHERE id = :id'; 
    $stmt = $db->prepare ($sql); 
    $id = (int)$orderId;  
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
    $stmt->bindParam(':customerName', $form_dat['customerName']);    
    $stmt->bindParam(':orderFoodName', ($form_dat['orderFoodName']));
